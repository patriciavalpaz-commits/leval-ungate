...
export interface SolucionProps {
  ...
  /** Visual de apoyo opcional (mockup/foto del mecanismo) — entre los pasos y el antes/después. */
  imagen?: ReactNode;
  id?: string;
}
  ...
  return (
    <SectionShell id={id} elevacion="base" ariaLabel="Cómo funciona">
      ...
      >
        ...
        {/* El chip del mecanismo bautizado — pill sólido con tinte del acento (55 §4) */}
        <motion.div variants={item} className="mt-4">
          <span
            className="inline-block w-fit rounded-full px-4 py-2 text-[15px] font-semibold"
            style={{
        ...
        {imagen && (
          <motion.div
            variants={item}
            className="mt-10 overflow-hidden rounded-[var(--radius-card)] border border-[color-mix(in_oklab,var(--text-tertiary)_15%,transparent)] shadow-[var(--shadow-2)]"
          >
            {imagen}
          </motion.div>
        )}
