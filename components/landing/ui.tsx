...
export function SectionShell({
  ...
  // Profundidad SIEMPRE (DESIGN-CORE §2: "nunca fill plano"): dos mesh radiales muy
  // sutiles del acento, atados a la alternancia base/elevada que ya define 55 T1 —
  // así el lado del degradé se alterna solo, sección por sección, sin repetirse.
  const blobs =
    elevacion === 'elevada'
      ? 'radial-gradient(640px 380px at 92% -8%, color-mix(in oklab, var(--accent) 10%, transparent) 0%, transparent 58%), ' +
        'radial-gradient(560px 360px at 4% 108%, color-mix(in oklab, var(--accent) 7%, transparent) 0%, transparent 55%)'
      : 'radial-gradient(640px 380px at 6% -8%, color-mix(in oklab, var(--accent) 8%, transparent) 0%, transparent 58%), ' +
        'radial-gradient(560px 360px at 96% 108%, color-mix(in oklab, var(--accent) 6%, transparent) 0%, transparent 55%)';
  return (
    <section
      id={id}
      aria-label={ariaLabel}
      className={`relative ${pt} ${pb} ${className}`}
      style={{
        background: `${blobs}, ${elevacion === 'elevada' ? 'var(--surface)' : 'var(--bg)'}`,
      }}
    >
