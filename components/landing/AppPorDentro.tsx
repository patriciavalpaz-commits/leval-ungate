...
export interface FrameCarrusel {
  ...
  src?: string;
  alt?: string;
  /** Contenido custom que llena el frame (ej. <VideoDemo>) — manda sobre src/placeholder. */
  media?: ReactNode;
  ...
  /** Ícono del placeholder (55: ancla visual, nunca el frame vacío de puro texto). */
  icon?: LucideIcon;
}
                ...
                >
                  {f.media ? (
                    ...
                    /* Placeholder honesto (55 §5.2): gris elevado + ÍCONO + nombre + dashed —
                       nunca un frame que finja producto terminado, nunca puro texto sin ancla visual */
                    <div className="flex h-full w-full flex-col items-center justify-center gap-3 border-2 border-dashed border-[color-mix(in_oklab,var(--text-tertiary)_40%,transparent)] bg-[var(--surface-2)] px-4">
                      {f.icon && <IconChip icon={f.icon} tone="muted" />}
